...
async def generate_summary_title(claims: list):
    ...
    Focus only on the main subject and event. Respond ONLY with the raw title string. Do not include quotes, markdown accents, or concluding periods.
...
async def analyze_pdf(file: UploadFile = File(...)):
    ...
    try:
        ...
        contents = await file.read() # Read the entire file into memory as bytes
        pdf_stream = BytesIO(contents) # Create a byte stream for PDF reader to process
        reader = PdfReader(pdf_stream) # Initialize PDF reader with the byte stream input
